# AGENTS.md

이 파일은 이 저장소에서 작업하는 Codex, Claude 및 기타 코딩 에이전트가 따라야 할 공통 규칙의 원본이다.

## 프로젝트 목적

`beaulead-agency-tools`는 고객 제안과 대면 미팅에서 사용할 디지털 에이전시용 인터랙티브 아티팩트를 관리하는 Public GitHub Pages 저장소다.

## 공개 저장소 안전 규칙

- 고객 개인정보, 담당자 연락처, 광고 계정 ID, 인증정보, API 키, 계약서, 비공개 성과 데이터, 내부 원가·마진을 커밋하지 않는다.
- 고객 사례에는 공개 승인을 받은 정보 또는 명확히 가공된 샘플 데이터만 사용한다.
- 실제 고객 데이터나 로그인이 필요한 기능은 이 저장소에 구현하지 말고 별도의 비공개 시스템으로 분리한다.
- `.env`, 로컬 캐시, 운영 데이터 파일은 커밋하지 않는다.
- 화면에 입력한 조건을 **밖으로 내보내는 기능**(견적 조건 URL 공유, `localStorage` 저장, 외부 전송 등)을 붙일 때 고객사명·페이지명처럼 어느 건인지 식별되는 텍스트 입력은 직렬화 대상에서 뺀다. 숫자 조건만 담는다. 링크는 전달되는 동안 복사되고 남으므로, 링크 자체가 "어느 고객사에 얼마를 불렀는지" 가 되면 안 된다. 2026-09-21 기준 그런 경로는 하나도 없다(`URLSearchParams`·`localStorage`·`fetch`·`<form action>` 전부 0건) — 처음 만드는 사람이 이 규칙의 첫 적용 대상이다.

## 구조

<!-- registry:structure:start — data/tools.json 에서 생성한다. 직접 고치지 말 것. -->
- `/index.html` — 에이전시 메인 홈페이지와 GNB
- `/video-quote/` — 영상 제작 견적기
- `/development-quote/` — 홈페이지·랜딩 수정 및 상세페이지 개발 예상 견적기 (저장소에만 있음 — 배포 제외)
- `/google-ads-training-quote/` — Google Ads 교육 상품 예상 견적기 (저장소에만 있음 — 배포 제외)
- `/youtube-creator-quote/` — 카페24·유튜브 쇼핑 크리에이터 제휴 견적기
- `/youtube-view-quote/` — 유튜브 조회수 보장형 광고 상품 가격표 (저장소에만 있음 — 배포 제외)
- `/keyword-volume-plan/` — 네이버 월간 검색량 조회 화면기획서
- `/ad-quote/` — 광고 운영 견적 (예정 — 아직 페이지 없음)
- `/content-quote/` — 콘텐츠 제작 견적 (예정 — 아직 페이지 없음)
- `/media-plan/` — 미디어 플랜 (예정 — 아직 페이지 없음)
- `/campaign-planner/` — 캠페인 플래너 (예정 — 아직 페이지 없음)
- `/reporting-dashboard/` — 성과 대시보드 (예정 — 아직 페이지 없음)
- `/client-demos/` — 공개 가능한 고객사별 시연본 (예정 — 아직 페이지 없음)
- `/work/` — 고객사 레퍼런스 목록
- `/shared/` — 공통 스크립트와 브랜드 자산
- `/data/` — 레지스트리 등 생성 원본 데이터 (저장소에만 있음 — 배포 제외)
- `/docs/` — 운영 문서 (저장소에만 있음 — 배포 제외)
- `/scripts/` — 저장소 검증·생성 도구 (저장소에만 있음 — 배포 제외)
<!-- registry:structure:end -->

위 목록은 `data/tools.json` 레지스트리에서 생성한다. 디렉토리를 추가·삭제하거나 이름·상태를 바꿀 때는 그 파일만 고치고 `python3 scripts/sync_registry.py --write`를 실행한다. 목록을 손으로 고치면 검증이 실패한다.

새 아티팩트는 목적에 맞는 형제 디렉토리의 `index.html`로 만들고 같은 커밋에서 레지스트리의 `status`를 `live`로 올린다. 루트 `index.html`을 개별 아티팩트로 덮어쓰지 않는다.

`public`을 내리면 배포에서 빠지는 것과 함께 루트 `index.html`의 카드도 사라지고 위 구조 목록에 `(저장소에만 있음 — 배포 제외)` 표시가 붙는다. 카드를 따로 지울 필요가 없고, 지우려 하면 레지스트리와 어긋나 검증이 실패한다. 공개 범위를 바꾸는 레버는 `public` 하나다.

## 작업 원칙

- `main`은 항상 고객에게 시연 가능한 상태로 유지한다.
- 작업은 `feature/<짧은-기능명>`, 수정은 `fix/<짧은-문제명>` 브랜치에서 시작한다.
- 변경은 Pull Request로 검토한 뒤 `main`에 병합한다. 긴급 상황을 제외하고 `main`에 직접 push하지 않는다.
- 사용자가 제공하지 않은 가격, 예산, 성과 수치나 승인 기준을 임의로 만들지 않는다.
- 고객사·서비스마다 달라질 값은 HTML/JavaScript에 반복 하드코딩하지 말고 해당 아티팩트의 설정 데이터로 분리한다.
- 색은 `~/brand-design-system`의 확정 토큰만 쓴다. 새 색을 지어내지 않는다. `shared/tokens.css`와 `data/brand-tokens.json`은 `python3 scripts/build_tokens.py`가 만드는 생성물이라 손으로 고치지 않는다. 페이지가 `shared/tokens.css`를 링크하면 `scripts/check_colors.py`의 검사 대상이 되고, 토큰 밖 색이 있으면 검증이 실패한다.
- 색 조합은 명암비를 지킨다. 어두운 배경에서 액센트(#0F62FE)는 3.62:1이라 본문 글자에 쓸 수 없고 면·테두리·큰 글씨에만 쓴다. 상태색 3종은 글자색이 아니라 15% 틴트 배경 + 잉크 글자 형태로만 쓴다. 근거와 수치는 `scripts/test_contrast.py`가 고정한다.
- 요청 범위 밖의 기존 아티팩트는 수정하지 않는다.
- 공통 코드를 바꿀 때는 변경 대상이 아닌 아티팩트가 이전과 동일한지 실제 파일 비교 또는 체크섬으로 확인한다.
- 공개 링크, 내부 링크, 모바일 레이아웃, 키보드 접근성을 배포 전에 확인한다.

## 필수 검증

변경 후 아래 명령을 실행한다.

```sh
python3 scripts/validate_site.py
git diff --check
```

변경 대상이 아닌 아티팩트가 있다면 작업 전후 체크섬도 비교한다.

```sh
shasum -a 256 path/to/index.html
```

검증 결과를 Pull Request 설명에 기록한다. 테스트 통과만으로 완료 처리하지 말고 변경된 페이지를 로컬 또는 배포 주소에서 실제로 열어 확인한다.

## 배포

- GitHub Pages는 `.github/workflows/pages.yml`이 배포한다. 저장소 루트를 통째로 서빙하지 않는다.
- 무엇이 공개될지는 `data/tools.json`의 `public` 필드가 정한다. `scripts/build_site.py`가 그 값을 읽어 `_site/`로 모으고 그 결과만 배포된다. 공개 범위를 바꾸려면 워크플로가 아니라 레지스트리를 고친다.
- `AGENTS.md`, `CLAUDE.md`, `README.md`, `docs/`, `scripts/`, `data/`와 각 디렉토리의 `README.md`는 배포되지 않는다. 이 경계는 `scripts/test_build_site.py`가 고정한다.
- 공개 저장소이므로 **배포되지 않는다는 것과 비공개라는 것은 다르다.** 커밋한 것은 GitHub에서 누구나 읽을 수 있다. 위쪽 「공개 저장소 안전 규칙」이 그대로 우선한다.
- Pull Request 검증이 통과하고 검토가 끝난 뒤에만 `main`에 병합한다.
- 병합 후 GitHub Pages 배포 완료와 공개 URL의 HTTP 200 응답을 확인한다.
- 대면 미팅에 사용할 확정본은 `demo-YYYY-MM-DD-이름` 형식의 Git 태그로 남긴다.

자세한 작업 및 복구 절차는 `docs/WORKFLOW.md`를 따른다.
