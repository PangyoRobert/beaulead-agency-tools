...
- 색을 쓸 때는 `shared/tokens.css`의 `--bl-*` 변수만 쓴다. 원본은 `~/brand-design-system/brands/beaulead/tokens.json`이고, 그 파일이 바뀌면 `python3 scripts/build_tokens.py`로 다시 생성한다. CI에는 그 레포가 없어서 재생성하지 못하므로 생성물을 커밋한다.
